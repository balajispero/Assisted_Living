...
class Patient extends General{
	...
	public function patient_master($offset = 0){
		...
		{	
			...
			$edit=anchor('app/patient/edit/'.$patient->patient_no,'Modify');
				...
				$edit=anchor('app/patient/edits/'.$patient->patient_no,'Modify');
			}
				$this->table->add_row( 
	...
	public function view($id){
		...
		$this->data['medicine'] = $this->patient_model->getmedicine($id);
	...
	public function save(){
		...
		/*********************End upload image code*********************/	
			...
			General::logfile('Member Registration','INSERT',$value);
	...
	public function edit($id = 0){
		...
		if(isset($_POST['btnSubmit'])){
			...
			$this->data['patientInfo'] = $this->patient_model->getPatient($id); 
			$this->load->view('app/patient/editPatients',$this->data);	
		}
	}
	...
	// public function addAssesForm($id){
	// 	$this->data['patient_no'] = $id;
	// 	$this->data['message'] = $this->session->flashdata('message');
	// 	$this->load->view('app/patient/addAssesForm',$this->data);	
	// }
	...
	// 	$config = array(
	// 				'allowed_types'		=>		'*',
	// 				'upload_path'		=>		realpath('public/patient_assesment_form'),
	// 				'max_size'			=>		3000
	// 				);
	...
	// 	if(!$this->upload->do_upload()){
	// 		$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>".$this->upload->display_errors()."</div>");
	// 		redirect(base_url().'app/patient/addAssesForm/'.$this->input->post('patient_no'),$this->data);
	// 	}else{
			
	// 		$image_data = $this->upload->data();
	// 		$this->patient_model->uploadform($form_data,$this->input->post('patient_no'));
	// 		$this->session->set_flashdata('message',"<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Attachment successfully Uploaded</div>");
	// 		redirect(base_url().'app/patient/addAssesForm/'.$this->input->post('patient_no'),$this->data);
	// 	}
	// }
