...
class Emr extends General{
	...
	public function opd($offset = 0){
		...
		{	
				if($patient->nStatus == "Pending"){ 
					$nStatus = "Pending";
					$discharge = anchor('app/ipd/admit/'.$patient->IO_ID.'/'.$patient->patient_no,'Discharge',array('class'=>'delete','onclick'=>"return confirm('Are you sure want to Admit Member?')"));
				}else{ 
					$nStatus = "Discharged";
	...
	public function ipd($offset = 0){
        ...
        $this->table->set_template($tmpl);
		$this->table->set_empty("&nbsp;");
		$this->table->set_heading('IPD No','Member No','Member Name','Date Admit','Department','Room & Bed No.','Incharge Doctor','Status');
		$i = 0 + $offset;
